    ...
    <section v-if="resultItems?.length">
            ...
            >
              <template v-if="item[item2.value] || item[item2.value] !== ''">
                ...
                <template v-else-if="item2.type === 'image'">
                  <nuxt-img
                    :preset="item2.preset || 'adminThumbnail'"
            ...
            <td class="py-2 border-b" width="10%">
              <div class="flex flex-col">
                <button
                  ...
                  class="bg-classic-rose-600 text-grayscale-200 rounded-md mb-1 py-1"
                  v-if="col.removeable"
                >
                  삭제
                </button>
                <button
                  class="bg-baja-white-500 text-grayscale-200 rounded-md py-1"
                  @click="
                    $router.push({
      ...
      <section class="mt-3">
        <nav class="footer-util">
          <button
            @click="viewMore()"
            v-if="!pending && !endPage && col.viewMoreable"
            class="inline-block border rounded-full px-8 py-3 border-grayscale-600 hover:animate-wiggle hover:bg-main-500 hover:text-grayscale-200 hover:border-main-500"
          >
            더 불러오기
          </button>
